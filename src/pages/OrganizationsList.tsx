...
const OrganizationsList = () => {
    ...
    const GetOrganizationData = (showLoader: boolean) => {
        ...
        axios.post(API_ENDPOINTS.AllOrganizations, formData, { headers })
            .then(response => {
                if (response.status == 200) {
                    // console.log('AllOrganiation===>', response?.data?.data);
    ...
    const DeleteOrg = (id: any) => {
        ...
        axios.delete(API_ENDPOINTS.ORGDELETE, {
            headers: headers,
            data: formData
        })
            .then(response => {
        ...
        const delayDebounceFn = setTimeout(() => {
            const filteredData = initialRecords.filter(item =>
                item.email.toLowerCase().includes(search.toLowerCase())
                 ||
                item.adminVerified.toLowerCase().includes(search.toLowerCase())
                 ||
    ...
    return (
        <div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl">Organizations List</h2>
                <div className="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                    <div className="flex gap-3"></div>
                    <div className="relative">
                        <div className="ltr:ml-auto rtl:mr-auto">
                            <input type="text" className="form-input w-auto" placeholder="Search Organizations" value={search} onChange={(e) => setSearch(e.target.value)} />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="datatables mt-5 panel overflow-hidden">
                ...
                <DataTable
                    ...
                    columns={[
                        ...
                        {
                            accessor: 'email', sortable: true, textAlignment: 'right',
                        },
                        { accessor: 'registrationType', title: 'Registration Type', sortable: true },
                        {
                            ...
                            render: ({ status, _id }) => {
                                ...
                                const handleToggle = () => {
                                    ...
                                    fetch(API_ENDPOINTS.ORG_STATUS_UPDATE, requestOptions)
...

                        {
                            ...
                            title: 'Verification',
                            sortable: true,
                            render: ({ adminVerified, _id }) => {
                                const [isChecked, setIsChecked] = useState(false);
                                const handleStatus = (value: any, adminVerified: any) => {
                                    // setIsChecked(value);
                                    setLoader(true)
                                    ...
                                    formdata.append("id", _id);
                                    formdata.append("adminVerified", adminVerified);
                                    ...
                                    fetch(API_ENDPOINTS.ORG_VERIFY, requestOptions)
                                        .then(response => response.json())
                                        .then(result => {
                                            if (result.status === 200) {
                                                GetOrganizationData(false);
                                ...
                                return (
                                    <div className="flex gap-4 items-center justify-center" onClick={(e) => e.stopPropagation()}>
                                        {adminVerified === 'In progress' ? (
                                            <label className="switch">
                                                <input
                                                    type="checkbox"
                                                    name='status'
                                                    checked={isChecked}
                                                    onChange={() => handleStatus(!isChecked, adminVerified)}
                                                />
                                                <span className="slider round"></span>
                                            </label>
                                        ) : (
                                            <p className={`rounded px-2 ${adminVerified === 'Verified' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                                                {adminVerified}
                                            </p>)}
                                    </div>
                                );
